"""Fee reminders — the cron entrypoint.

Fires whichever rung of each school's escalation ladder is due, for every fee
still outstanding. Safe to run often: each rung fires at most once per fee,
enforced by a unique constraint, so an hourly schedule and a manual re-run
after a failure both do the right thing.

Off by default per school: acts only on a tenant whose central
"fee_reminders" platform feature flag is on, the same gating as the fee,
promotion and exam schedulers. A module that messages parents must not
switch itself on.
...
Usage:
  python run_fee_reminders.py
  python run_fee_reminders.py --dry-run          # report who would be contacted
  python run_fee_reminders.py --as-of 2026-09-01 # pretend it is another day
  python run_fee_reminders.py --limit 50         # smaller batch
"""
...
from app import fee_reminders  # noqa: E402
...
logger = logging.getLogger("fee_reminders")
FEATURE_KEY = "fee_reminders"
...
def run_tenant(database_url: str, as_of: date | None, dry_run: bool, limit: int) -> dict:
    ...
    db = factory()
    try:
        return fee_reminders.run_reminders(db, as_of=as_of, dry_run=dry_run, limit=limit)
    finally:
        db.close()


def main() -> int:
    parser = argparse.ArgumentParser(description="Send due fee reminders")
    ...
    parser.add_argument("--limit", type=int, default=fee_reminders.DEFAULT_BATCH_LIMIT,
