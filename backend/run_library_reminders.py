"""Overdue library book reminders — the cron entrypoint.

Fires whichever rung of each school's escalation ladder is due, for every
book still out and overdue. Safe to run often: each rung fires at most once
per issue, enforced by a unique constraint, so an hourly schedule and a
manual re-run after a failure both do the right thing.

Off by default per school: acts only on a tenant whose central
"library_reminders" platform feature flag is on, the same gating as
fee_reminders. A module that messages parents/staff must not switch itself
on.

Once a day is the sensible schedule. The rungs are day-grained, so running
hourly changes nothing except which hour the message lands in.

Usage:
  python run_library_reminders.py
  python run_library_reminders.py --dry-run          # report who would be contacted
  python run_library_reminders.py --as-of 2026-09-01 # pretend it is another day
  python run_library_reminders.py --limit 50         # smaller batch
"""
...
from app import library_reminders  # noqa: E402
...
logger = logging.getLogger("library_reminders")
FEATURE_KEY = "library_reminders"
...
def run_tenant(database_url: str, as_of: date | None, dry_run: bool, limit: int) -> dict:
    ...
    db = factory()
    try:
        return library_reminders.run_reminders(db, as_of=as_of, dry_run=dry_run, limit=limit)
    finally:
        db.close()


def main() -> int:
    parser = argparse.ArgumentParser(description="Send due overdue-library-book reminders")
    parser.add_argument("--dry-run", action="store_true",
                        help="report who would be contacted, send nothing")
    parser.add_argument("--as-of", help="run as though it were this date (YYYY-MM-DD)")
    parser.add_argument("--limit", type=int, default=library_reminders.DEFAULT_BATCH_LIMIT,
                        help="most messages to send in one run")
    ...
    for account_code, database_url in tenant_accounts():
        ...
        logger.info(
            "%s: considered=%s sent=%s skipped=%s failed=%s superseded=%s%s",
            account_code, result.get("considered", 0), result.get("sent", 0),
            result.get("skipped", 0), result.get("failed", 0),
            result.get("superseded", 0), " (dry run)" if args.dry_run else "",
        )
        if result.get("note"):
            logger.info("%s: %s", account_code, result["note"])
